...
export default function CalendarPage() {
  ...
  async function fetchEvents() {
    ...
    const { data: oshiList } = await (supabase as any)
      .from("oshi").select("id").eq("user_id", user!.id);

    const oshiIds: string[] = (oshiList ?? []).map((o: { id: string }) => o.id);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", paddingBottom: 80 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 1, background: "var(--color-border)", flexShrink: 0 }}>
        {days.map((day) => {
          ...
          return (
            <button key={key}
              ...
              style={{
                ...
                minHeight: 56, display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
              }}
            >
              <span style={{
                width: 26, height: 26, display: "flex", alignItems: "center", justifyContent: "center",
                borderRadius: "50%", fontSize: 13, fontWeight: today ? 700 : 400,
                background: today ? "var(--color-primary)" : "transparent",
                color: today ? "#fff" : !inMonth ? "var(--color-text-3)" : dow === 0 ? "#ef4444" : dow === 6 ? "#3b82f6" : "var(--color-text)",
              }}>
                {format(day, "d")}
              </span>
        ...
        {selectedDate ? (
          <>
            ...
            {selectedEvents.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {selectedEvents.map((e) => <DayEventItem key={e.id} event={e} />)}
              </div>
            )}
          </>
        ) : (
          <div>
            ...
            {events.length === 0 && !loading && (
              <p style={{ color: "var(--color-text-3)", fontSize: 14 }}>今月の予定はありません</p>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{
        ...
        display: "flex", gap: 16, background: "var(--color-surface)",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--color-text-2)" }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--color-live)" }} />ライブ
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--color-text-2)" }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--color-release)" }} />リリース
        </div>
      </div>
    </div>
  );
...
function DayEventItem({ event, showDate }: { event: EventWithOshi; showDate?: boolean }) {
  ...
  return (
    <a href={event.url ?? "#"} target={event.url ? "_blank" : undefined} rel="noopener noreferrer"
      style={{
        background: "var(--color-surface)",
...
const navBtnStyle: React.CSSProperties = {
  ...
  fontSize: 20, cursor: "pointer", color: "var(--color-text-2)",
